...
preprocessing_experiments.append(
    {"name": "reduce noise", "preprocessing": {"reduce_noise": True}}
)
preprocessing_experiments.append(
    {"name": "overlay", "preprocessing": {"use_overlay": True}}
)
experiments.extend(preprocessing_experiments)

n_points_experiments = [
    {"name": f"{n} points", "data": {"max_points": n}} for n in (8, 16, 32, 64, 128)
]
experiments.extend(n_points_experiments)

training_set_size_experiments = [
    {"name": f"{n} samples", "data": {"n_train_samples": n}}
    for n in (100, 500, 1000, 5000, 10_000, 50_000, None)
]
...
for job_index, start in enumerate(range(0, len(experiments), experiments_per_job)):
    ...
    for i, experiment_cfg in enumerate(
        islice(experiments, start, start + experiments_per_job)
    ):
